...
let cartDelegationInitialized = false;
let checkoutDelegationInitialized = false;
...
function addToCart(productId, quantity = 1) {
    ...
    if (existingItem) {
        ...
        updateCartItemElement(existingItem);
...
function removeFromCart(productId) {
    ...
    if (itemIndex > -1) {
        ...
        const removedElement = document.querySelector(`.cart-item[data-product-id="${id}"]`);
        if (removedElement) {
...
function updateCartItemElement(item) {
    ...
    const quantityInput = cartItem.querySelector('.cart-qty-input');
    if (quantityInput) {
...
function setupCartModal() {
    ...
    if (!cartIcon || !cartModal || !closeCartBtn || !continueShoppingBtn) {
        console.log('Cart modal elements are not ready yet, will retry later');
        return false;
    }
    
    // Delegate click events for cart item controls
    if (!cartDelegationInitialized) {
        ...
        cartDelegationInitialized = true;
    }

    // Open cart modal
    if (!cartIcon.dataset.listenerAttached) {
    ...
    if (!continueShoppingBtn.dataset.listenerAttached) {
